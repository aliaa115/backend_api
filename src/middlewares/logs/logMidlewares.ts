...
globalThis.console_log = (...msg: any[]): void => {
  console.log(colors["RESET"], ...msg);
};
globalThis.console_error = (...msg: any[]) => {
  msg.forEach((data) => {
    if (typeof data == "object") data = JSON.stringify(data, null, 2);
    console.error(colors["RED"], data);
  });
};
globalThis.console_catch = (msg: any) => {
  let e = get_error_conent(msg);
  if (typeof e == "object") {
    e.message && console.error(colors["RED_BRIGHT"], e.message);
    e.sub_message && console.error(colors["YELLOW"], e.sub_message);
    e.stack && console.error(colors["RED"], e.stack);
  } else console_error(e);
};
globalThis.console_warn = (...msg: any[]) => {
  msg.forEach((data) => {
    if (typeof data == "object") data = JSON.stringify(data, null, 2);
    console.log(colors["YELLOW"], data);
  });
};
globalThis.console_info = (...msg: any[]) => {
  msg.forEach((data) => {
    if (typeof data == "object") data = JSON.stringify(data, null, 2);
    console.log(colors["CYAN"], data);
  });
};
globalThis.console_time = (...msg: any) => {
  console.time(msg);
};
